...
const AllSearch = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setloading(true);
      let dataToSearch = location.state.searchParms;
      let { data } = await axios.post(
        `${apiUrl}/user/searchgigs`,
        dataToSearch
      );
  ...
  return (
    <div>
      <Navbar />
      <Link className="text-sm text-blue-500 ml-11" to={"/"}>
        &larr; Home
      </Link>
      <div className="p-5 text-center">
        ...
        <h2>{allGigs && allGigs.length === 0 && "No Data"}</h2>
                  ...
                  val.userInfo._id !== userid ? (
                    <div className="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 cursor-pointer">
                      ...
                      <div className="flex flex-col items-center pb-10">
                        ...
                        <span className="text-sm text-gray-500 dark:text-gray-400 font-bold my-2">
                          {val.category}
                        </span>
