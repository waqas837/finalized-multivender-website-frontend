...
function BookingOrder({ fromMyProfile }) {
  ...
  const getAllOrder = async () => {
    ...
    let { data } = await axios.get(`${apiUrl}/user/getAllBueryOrders/${buyerParseddata._id}`)
    if (data.success) {
  ...
  const projectMarkAscompletedByBuyer = async (OrderId,price) => {
    try {
      let { data } = await axios.get(`${apiUrl}/user/buyerOrderCompleted/${OrderId}`)
      if (data.success) {
        console.log("data.allOrders", data.allOrders);
  ...
  const handleSubmit = async (e) => {
    ...
    let { data } = await axios.post(`${apiUrl}/user/orderRequestByBuyer/${buyerParseddata._id}/${sellerid}/${gigid}`, Adata)
    if (data.success) {
      setrender(!render)
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-6">
      ...
      {!Order && (
        <div>
          {!fromMyProfile && <><h1 className="text-3xl font-bold mb-4 text-center">
            Book Your Order
          </h1><form
      ...
      <section class="py-24 relative">
        <div class="w-full max-w-7xl px-4 md:px-5 lg-6 mx-auto">
          ...
          <p class="mt-4 font-normal text-lg leading-8 text-gray-500 mb-11 text-center">
            Your order with other sellers shown below with their respective status
          </p>
          <div class="m-auto font-bold">
            <h1 className="text-2xl text-center">Orders List</h1>
          </div>
          {OrderDetails && OrderDetails.map((val, index) => (
            <div key={index} class="main-box border border-gray-200 rounded-xl pt-6 max-w-xl max-lg:mx-auto lg:max-w-full shadow-lg">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 w-full">
                      <div class="flex items-center">
                        <div class="">
                          ...
                          <button onClick={() => navigate(`/seller/${val.sellerId._id}`, { state: { userVisit: true } })} class="font-normal text-lg leading-8 text-gray-500 mb-3 ">
                          ...
                          <div class="flex items-center">
                            <p class="font-medium text-base leading-7 text-black pr-4 mr-4 border-r border-gray-200">
                              ...
                              <button onClick={() => val.acceptedBySeller && val.markAsCompleteBySeller && setShowFile(true)} class="bg-green-100 text-green-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-green-400 border border-green-400">
                                <div className="flex items-center">
                                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M17.25 8.25 21 12m0 0-3.75 3.75M21 12H3" />
                                  </svg>

                                  {val.acceptedBySeller && !val.markAsCompleteBySeller ? "Offer Accepted" : !val.acceptedBySeller && !val.markAsCompleteBySeller ? "Offer is on the way" : val.acceptedBySeller && val.markAsCompleteBySeller ? "Collect the project files And Rate The Seller" : ""}
                                </div></button>
                            </p>
                        ...
                        <div class="col-span-5 lg:col-span-2 flex items-center max-lg:mt-3 ">
                          <div class="flex gap-3 lg:block">
                            ...
                            <p class="font-medium text-sm leading-6 whitespace-nowrap py-0.5 px-3 rounded-full lg:mt-3 bg-emerald-50 text-emerald-600">
                              {val.markAsCompleteBySeller && !val.markAsCompleteByBuyer ? "Seller Delivered Project" : val.markAsCompleteBySeller && val.markAsCompleteByBuyer ? "Project Completed & Closed" : "Pending"}
                            </p><br />
                          </div>
                        </div>
                        <div class="col-span-5 lg:col-span-2 flex items-center max-lg:mt-3">
                          <div class="flex gap-3 lg:block">
                            ...
                            <p class="font-medium text-base leading-7 lg:mt-3 text-emerald-500">
                              {val.acceptedBySeller ? <RemainingTime remainTime={val.deliveryTime} /> : "Waiting for seller"}
                            </p>
                          </div>
                        </div>
                      </div>
              ...
              <div>
                {ShowFile && val.markAsCompleteBySeller &&
                  <div className="my-3 shadow-lg border-2 rounded-md text-center">
                    <h1 className="text-lg font-bold">Project Files sent by Seller</h1>
                    <h2>Please collect the delivery</h2>
                    <section className="">
                      {val.projectFilePaths.map((val, index) => (
                        <div key={index}>
                          <div className="flex justify-center items-center">
                            {val} <a download={`${apiUrl}/images/${val}`} href={`${apiUrl}/images/${val}`}>
                              <div className="flex justify-around w-32">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6">
                                  <path fill-rule="evenodd" className="text-blue-400" d="M19.5 21a3 3 0 0 0 3-3V9a3 3 0 0 0-3-3h-5.379a.75.75 0 0 1-.53-.22L11.47 3.66A2.25 2.25 0 0 0 9.879 3H4.5a3 3 0 0 0-3 3v12a3 3 0 0 0 3 3h15Zm-6.75-10.5a.75.75 0 0 0-1.5 0v4.19l-1.72-1.72a.75.75 0 0 0-1.06 1.06l3 3a.75.75 0 0 0 1.06 0l3-3a.75.75 0 1 0-1.06-1.06l-1.72 1.72V10.5Z" clip-rule="evenodd" />
                                </svg>
                                <span>Download</span>
                              </div>
                            </a>
                      ...
                      <div className="flex justify-around">
                        <button
                          onClick={() => projectMarkAscompletedByBuyer(val._id,val.Price)}
                          type="submit"
                        ...
                        <button
                          onClick={() => rateSeller(val._id, val.gigId, val.sellerId._id, val.buyerId)}
